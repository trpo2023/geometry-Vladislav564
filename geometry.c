...
int main() {
  ...
  fptr = fopen("point.txt", "r");
  fptr2 = fopen("point2.txt", "r");
  if (!fptr || !fptr2) {
  ...
  if (radius - radius2 < sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1)) &&
    ...
    printf("Figure intersects = Rock and stone!\n");
  else
    printf("Figure doesn't intersect = mission failed\n");
